...
int populateSi_siiPDO(uint16 slave, uint8 t, int mapoffset, int bitoffset,  ecx_contextt *context)
{
    uint16 a , w, c, e, er, Size;
    uint8 eectl;
    uint16 obj_idx;
    ...
    if (PDO->Startpos > 0)
    {
        ...
        do
        {
            ...
            if (PDO->SyncM[PDO->nPDO] < EC_MAXSM) /* active and in range SM? */
            {
                ...
                if (t) {
                    // printf("  SM%1d RXPDO 0x%4.4X %s\n", PDO->SyncM[PDO->nPDO], PDO->Index[PDO->nPDO], str_name);
                    context->slavelist[slave].ec_sii[t].SM = PDO->SyncM[PDO->nPDO];
                    context->slavelist[slave].ec_sii[t].Address = PDO->Index[PDO->nPDO];
                    strcpy(context->slavelist[slave].ec_sii[t].Name, str_name);
                }
                else{
                //   printf("  SM%1d TXPDO 0x%4.4X %s\n", PDO->SyncM[PDO->nPDO], PDO->Index[PDO->nPDO], str_name);
                  context->slavelist[slave].ec_sii[t].SM = PDO->SyncM[PDO->nPDO];
                ...
                for (er = 1; er <= e; er++)
                {
                    ...
                    //    printf("  [0x%4.4X.%1d] 0x%4.4X:0x%2.2X 0x%2.2X", abs_offset, abs_bit, obj_idx, obj_subidx, bitlen);
                       context->slavelist[slave].ec_sii[t].ec_sii_variable[er - 1].abs_offset = abs_offset;
                       context->slavelist[slave].ec_sii[t].ec_sii_variable[er - 1].abs_bit = abs_bit;
                       context->slavelist[slave].ec_sii[t].ec_sii_variable[er - 1].obj_idx = obj_idx;
                       context->slavelist[slave].ec_sii[t].ec_sii_variable[er - 1].obj_subidx = obj_subidx;
                       context->slavelist[slave].ec_sii[t].ec_sii_variable[er - 1].bitlen = bitlen;
                    //    printf(" %-12s %s\n", dtype2string(obj_datatype), str_name);
                       strcpy(context->slavelist[slave].ec_sii[t].ec_sii_variable[er - 1].dataType, dtype2string(obj_datatype));
                       strcpy(context->slavelist[slave].ec_sii[t].ec_sii_variable[er - 1].name, str_name);
                    }
                    bitoffset += bitlen;
                    totalsize += bitlen;
                }
   ...
   if (ec_init(ifname))
   {
      ...
      if ( ec_config(FALSE, &IOmap) > 0 )
      {
         ...
         for( cnt = 1 ; cnt <= ec_slavecount ; cnt++)
         {
            ...
            if ((ec_slave[cnt].mbx_proto & ECT_MBXPROT_COE) && TRUE)
                    si_sdo(cnt);
            //     if(TRUE)
            // {
